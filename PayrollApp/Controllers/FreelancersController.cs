...
namespace PayrollApp.Controllers
{
    ...
    {
        public FreelancersView FreelancersView { get; }
        public RepositoryService RepositoryService { get; }
        public User User { get; }
        public FreelancersController(FreelancersView freelancersView, RepositoryService repositoryService, User user)
        {
            FreelancersView = freelancersView;
            RepositoryService = repositoryService;
            User = user;
        }
        public void Run(string msg)
        {
            string action = FreelancersView.GetStartScreenAction(msg);
            switch (action)
            {
                case "1":
                    GetSelfReport(null);
        ...
        private void AddHours(string msg)
        {
            ...
            try
            {
                switch (option)
                {
                    case "1":
                        {
                            ReportNote reportNote = FreelancersView.GetThisDayAddHoursForm(User);
                    ...
                    case "2":
                        {
                            ReportNote reportNote = FreelancersView.GetAddHoursForm(User);
                            User user = RepositoryService.Database.Users.FirstOrDefault(u => u.Name == reportNote.UserName);
                            if (user == null) throw new ArgumentException("Пользователя с таким именем не сущестует.\nДобавьте его сначала в список пользователей\n");
                            RepositoryService.AddReportNote(reportNote, user.Role);
                            Run("Запись успешно добавлена" + '\n');
